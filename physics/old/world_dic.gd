...
const body = preload("res://physics/body_dic.gd")
const collision = preload("res://physics/collision_dic.gd")
...
class circle extends "body_dic.gd":
	var radius
...
func boxVScircle(a, b):
	...
	if vec2.Equal(relPos, closest):
		inside = true
		xDist = number.Sub(xExtent, number.Abs(closest.X))
		yDist = number.Sub(yExtent, number.Abs(closest.Y))
		if xDist < yDist:
	...
	if d == 0:
		if xDist == 0:
			c.normal = vec2.New(number.ONE if relPos.X > 0 else -number.ONE, 0)
		else:
			c.normal = vec2.New(0, number.ONE if relPos.Y > 0 else -number.ONE)
	else:
