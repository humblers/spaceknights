...
var patience_time = 80
var dice = 100
const PATIENCE_LIMIT = 300
const PATIENCE_LOWLEST = 100
...
func Update():
	...
	if game.step % 30 == 0:
		dice = (randi() % (PATIENCE_LIMIT - PATIENCE_LOWLEST) + PATIENCE_LOWLEST)
	var card_candidates = []
	for card in hand:
	...
	if len(card_candidates) == 0:
		patience_time += 1
		if patience_time < dice:
			return
	else:
		card = elect_best_card(card_candidates)
	...
	if patience_time > dice:
		for card in hand:
			if card == null:
				continue
			if make_aggression_for_use_card(card.Name, card.Level):
				card_aggression.append([card.Name, card.Level, data.SquireCard])
		if len(card_aggression) == 0:
			return
		card = card_aggression[randi() % len(card_aggression)]
		make_aggression_input(card)
		patience_time = 0
	patience_time += 1
...
func make_aggression_for_use_card(card, level):
	var cost = data.cards[card].Cost
	if cost > energy:
		return false
	if data.cards[card].Type == data.KnightCard:
		return false
	return true
...
func make_aggression_input(card):
	var name = card[0]
	var x = randi() % 20
	var y = randi() % 15
