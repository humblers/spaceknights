...
type response struct {
	...
	// DEPRECATED. remove will prior to go 1.11
	// https://tip.golang.org/doc/go1.11#net/http
...
func (w response) WriteHeader(code int) {
	...
	if cl := w.handlerHeader.Get("Content-Length"); cl != "" {
		...
		if err == nil && v >= 0 {
			...
			w.conn.logger.Printf("http: invalid Content-Length of %q", cl)
...
func (w *response) CloseNotify() <-chan bool {
	w.conn.closeChResp = make(chan bool)
	return w.conn.closeChResp
}
...
func (w *response) finishRequest() {
	...
	if w.conn.closeChResp != nil {
		close(w.conn.closeChResp)
		w.conn.closeChResp = nil
	}
...
func (cw *chunkWriter) flush() {
	...
	err := cw.writer.Close()
	if err != nil {
		cw.res.conn.logger.Print("cannot flush")
		return
	}
	w, err := cw.res.conn.nextWriter(websocket.TextMessage)
	if err != nil {
		cw.res.conn.logger.Print("cannot write to the connection")
		return
	}
	cw.writer = w
}
...
func (cw *chunkWriter) finalflush() {
	...
	if err := cw.writer.Close(); err != nil {
		cw.res.conn.logger.Printf("cannot flush(%v)", err)
	}
	cw.writer = nil
}
...
func (cw *chunkWriter) writeHeader(p []byte) {
	...
	if hasCL && hasTE && te != "identity" {
		cw.res.conn.logger.Printf("http: WriteHeader called with both Transfer-Encoding of %q and a Content-Length of %d",
			te, w.contentLength)
	...
	if w.req.Method == "HEAD" || !bodyAllowedForStatus(code) {
		...
		if hasTE && te == "identity" {
			...
			if hasTE && te == "chunked" {
				cw.res.conn.logger.Print("deleting transfer encoding")
