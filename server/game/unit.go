...
func (u *Unit) HandleSpawn() {
	...
	switch u.SpawnThing {
	case "footman":
		unit := NewFootman(0, u.Team, Vector2{0, 0}, Vector2{0, 0})
		unit.Position = getSpawnPos(unit.Radius)
		u.Game.AddUnit(NewFootman(0, u.Team, unit.Position, Vector2{0, 0}))
		u.Game.AddUnit(unit)
	case "sentry":
		unit := NewSentry(0, u.Team, Vector2{0, 0}, Vector2{0, 0})
		unit.Position = getSpawnPos(unit.Radius)
		u.Game.AddUnit(unit)
	case "pixie":
		unit := NewPixie(0, u.Team, Vector2{0, 0}, Vector2{0, 0})
		unit.Position = getSpawnPos(unit.Radius)
		u.Game.AddUnit(unit)
	case "knightbullet":
